...
func NewAppHandler(store *store.SessionStore, repo *repos.AppRepository) *AppHandler {
	return &AppHandler{repo:repo, store:store}
}
...
func (handler *AppHandler) UploadBlobHandler(w http.ResponseWriter, r *http.Request) {
	...
	blob, err := handler.repo.UploadBlob(account.ID, appName, file)
	if err != nil {
		JSON(w, 200, &Response{Error: true, Message: err.Error()})
		return
	}
